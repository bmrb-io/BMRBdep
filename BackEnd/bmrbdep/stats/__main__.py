...
opt.add_option("--fix-deposition-date", action="store_true", dest="fix_deposition_date", default=False,
               help="Fix a bug that existed in the deposition_date value in the submission_info.json file.")
...
if options.fix_deposition_date:
...
for entry in list_all_depositions():
    ...
    with DepositionRepo(entry, read_only=True) as entry_repo:
        if 'deposition_date' in entry_repo.metadata:
